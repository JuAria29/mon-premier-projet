...
export function CAProgressGauge({ exerciceDebut, exerciceFin, activites = [], caObjectif }: Props) {
  const [caPaye, setCaPaye] = useState(0);
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    setLoading(true);
    try {
      const params = new URLSearchParams({ statuts: "paid", limit: "1" });
      ...
      setCaPaye(data.summary?.paid?.total ?? 0);
    } finally {
      setLoading(false);
    }
  ...
  const pct = caObjectif > 0 ? Math.min((caPaye / caObjectif) * 100, 100) : 0;
  const restant = Math.max(caObjectif - caPaye, 0);

  // Mois restants dans l'exercice
  const moisRestants = exerciceFin
    ? Math.max(0, Math.ceil((new Date(exerciceFin).getTime() - Date.now()) / (30.44 * 24 * 3600 * 1000)))
    : null;

  // Nombre total de mois dans l'exercice (pour cible mensuelle)
      ...
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginBottom: 12, flexWrap: "wrap", gap: 8 }}>
        <div>
          ...
          <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
            <span style={{ fontSize: 36, fontWeight: 800, color: barColor, lineHeight: 1 }}>{fmtK(caPaye)}</span>
